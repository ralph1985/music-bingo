...
describe("admin Convex command", () => {
  ...
  it("reads the active game only from the administrative Convex endpoint", async () => {
    ...
    await expect(getActiveGameCommand({
      ...
      secret: "shared-test-secret",
      fetcher,
    })).resolves.toEqual({ calledSongIds: [], joinCode: "FIESTA", playlist: [], status: "waiting" });
  ...
  it("sends a finish command only to the administrative Convex endpoint", async () => {
    const fetcher = vi.fn().mockResolvedValue(new Response(JSON.stringify({ ok: true }), { status: 200 }));

    await expect(finishGameCommand({
      cloudUrl: "https://example.convex.cloud",
      secret: "shared-test-secret",
      joinCode: "FIESTA",
      fetcher,
    ...
    expect(fetcher).toHaveBeenCalledWith("https://example.convex.site/admin/games/finish", {
      method: "POST",
      headers: {
        "content-type": "application/json",
        "x-admin-command-secret": "shared-test-secret",
