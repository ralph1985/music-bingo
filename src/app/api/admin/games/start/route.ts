...
import { startGameCommand } from "@/server/convex/admin-command";
...
export async function POST(request: Request) {
  ...
  if (!cloudUrl || !secret) {
    return NextResponse.json({ error: "El inicio de partida no está configurado." }, { status: 503 });
  }
  if (!joinCode) {
  ...
  try {
    await startGameCommand({ cloudUrl, joinCode, secret });
    return NextResponse.json({ ok: true });
  } catch {
    return NextResponse.json({ error: "No se pudo iniciar la partida." }, { status: 422 });
  }
}
